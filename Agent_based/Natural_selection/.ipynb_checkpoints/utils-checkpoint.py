...
@njit
def reproduction(x, v, alpha, size, angle, energy, sight_radius, reproduction_threshold, reproduction_cost, life, steps, mutatables: list):
    for i in range(len(x)):
        if energy[i] >= reproduction_threshold:
            ...
            if 'alpha' in mutatables:
                new_alpha = np.random.normal(alpha[i], 0.1)
                while new_alpha < 0.1:
                    new_alpha = np.random.normal(alpha[i], 0.1)
                new_steps = np.round(np.random.pareto(new_alpha),0)
            else:
                new_alpha = alpha[i]
                new_steps = np.round(np.random.pareto(new_alpha),0)
    ...
    return x, v, alpha, size, angle, energy, sight_radius, life, steps
...
@njit
def update_position(x, v, alpha, dt, size, L, angle, rand, energy, food, food_energy, sight_radius, max_energy, life, metabolic_rate, steps):
    """
    Individuals move completely following a Levy flight until food comes inside their
    ...
    The Levy flight is modelled by choosing the length of time in which the individual
    will move balistically, until it changes direction. This flight period is given by
    ...
    Energy_cost = metabolism + A*(r² + v)

    where r is the sight_radius, A is a proportionality constant and v is the velocity
    """
    # print("Len: ", len(x))
    for i in range(len(x)):
        # print(i)
        if energy[i] <= 0:
            ...
            if len(food_in_sight) > 0 and len(food_eaten) == 0:
                ...
                if steps_remaining == 0:
                    new_steps = np.round(np.random.pareto(alpha[i]),0)
                    steps[i] = new_steps
            ...
            energy[i] -= metabolic_rate*dt
            energy[i] -= 0.5*(sight_radius[i]**2 + v[i])*dt
    ...
    return x, angle, energy, food, life, steps